...
import { updateEntity, deleteEntity, ServiceError } from '$lib/server/services';
...
export const PATCH: RequestHandler = async (event) => {
	...
	try {
		const result = await updateEntity({ ...body, uuid }, ctx);
		return json(result);
	} catch (err) {
...
export const DELETE: RequestHandler = async (event) => {
	...
	try {
		await deleteEntity({ uuid, incident_id: body.incident_id as string | undefined }, ctx);
